/*
 * cmd_prune.c — renatum prune --older-than <duration>
 *
 * v0.1.0 scope: --older-than only. The borg-style --keep-N-* flags,
 * --path prefix filter, and --dry-run are deferred.
 *
 * Pipeline:
 *   1. open the index for WRITE — this contends with renatumd if
...
int cmd_prune(int argc, char **argv, const rnt_config_t *cfg)
{
    ...
    while ((c = getopt_long(argc, argv, "t:Gh", opts, NULL)) != -1) {
        ...
        case 'h':
            fprintf(stderr,
                "Usage: renatum prune --older-than <duration> "
                "[--no-gc]\n");
            return RNT_EXIT_OK;
        default:
    ...
    if (older_than == NULL) {
        fprintf(stderr, "renatum prune: --older-than required\n");
        return RNT_EXIT_USAGE;
    }
    ...
    if (parse_duration_ns(older_than, &window_ns) < 0) {
        fprintf(stderr,
                "renatum prune: invalid duration '%s'\n", older_than);
        return RNT_EXIT_USAGE;
    }
    if (window_ns == 0) {
        fprintf(stderr,
                "renatum prune: 'infinite' makes no sense for --older-than\n");
        return RNT_EXIT_USAGE;
    }
    uint64_t cutoff = now_ns();
    if (cutoff < window_ns) {
        fprintf(stderr,
                "renatum prune: cutoff would go negative; clock skew?\n");
        return RNT_EXIT_ERROR;
    }
    cutoff -= window_ns;

    rnt_index_t *idx = index_open(cfg->index_path, RNT_IDX_WRITE);
    if (idx == NULL) {
        fprintf(stderr,
                "renatum prune: cannot open index for write "
                "(daemon running?): %s\n", strerror(errno));
        return RNT_EXIT_ERROR;
    }
    ...
    if (index_prune_by_age(idx, cutoff, &blobs_orphaned) < 0) {
        fprintf(stderr, "renatum prune: index_prune_by_age failed\n");
        index_close(idx);
        return RNT_EXIT_ERROR;
    }
    ...
    if (!no_gc && blobs_orphaned > 0) {
        ...
        if (st == NULL) {
            fprintf(stderr, "renatum prune: cannot open store for GC\n");
            index_close(idx);
            return RNT_EXIT_ERROR;
        }
        size_t removed = 0;
        if (retention_sweep_orphans(idx, st, &removed) < 0) {
            fprintf(stderr, "renatum prune: GC sweep failed\n");
            store_close(st);
            index_close(idx);
            return RNT_EXIT_ERROR;
        }
