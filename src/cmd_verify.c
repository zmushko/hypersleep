/*
 * cmd_verify.c — hypersleep verify
 *
 ...
 * The exit code follows docs/cli-spec.md: 0 on clean, HS_EXIT_CORRUPT
...
static int verify_cb(const uint8_t sha[HS_SHA_LEN], off_t size, void *user)
{
    (void)size;
...
int cmd_verify(int argc, char **argv, const hs_config_t *cfg)
{
    ...
    if (s == NULL) {
        fprintf(stderr, "hypersleep verify: cannot open store\n");
        return HS_EXIT_ERROR;
    }

    struct verify_acc acc = { s, 0, 0, 0 };
    if (store_iterate(s, verify_cb, &acc) < 0) {
        fprintf(stderr, "hypersleep verify: iterate failed\n");
        store_close(s);
        return HS_EXIT_ERROR;
    }
    store_close(s);
    ...
    if (acc.mismatches > 0) return HS_EXIT_CORRUPT;
    if (acc.errors > 0)     return HS_EXIT_ERROR;
    return HS_EXIT_OK;
}
